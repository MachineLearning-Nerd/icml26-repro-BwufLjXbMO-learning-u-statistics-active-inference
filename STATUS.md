...
- Current phase: Claim 5 official OpenReview supplement-access audit complete; decisive-claim policy reconciled
...
- Claim 5: Attempts 1–3 plus the required literal-source falsification attempt remain inconclusive; a fourth, distinct official OpenReview forum/note-API route found a Turnstile challenge and 403 public-note access, not evidence that no supplement exists. The accepted source supports the 20%-versus-classical statement, while no public source-faithful snapshot, case manifest, preprocessing, XGBoost configuration, seed, or 3,000-trial output is available. No proxy clinical evaluation was run.
...
- Next action: Claims 1–3 are terminal scoped verifications and Claim 4 is terminal literal falsification; monitor Claims 5–6 for new public source-faithful artifacts, then reconcile/regenerate the retained logbook draft and run independent review. Do not run redundant attempts for decisive claims.
- Publication status: blocked pending policy-reconciled logbook regeneration and independent review; prior local logbook/strict-poster/test/official-validator gates are draft-only and must not be published.
